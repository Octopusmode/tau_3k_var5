...
# Мощность двигателя
P = (1.5*Mст) * (n_ро * 60) / 0.975 # ~ 27.7 Вт
...
T_ос = 1 # TODO Уточнить
W_дос_p = 1 / (T_ос * ρ + 1)
W_ек_p = (k_му * (T_ос * ρ + 1)) / ((T_му * ρ + 1)*(T_ос * ρ + 1)+k_му)

## Расчет передаточной функции системы
# Коэффциент передачи разомкнутой системы
k_рс = k_тс * k_му * k_дв * k_р * k_ро * k_d * k_оу # * k_у

# W_рс_p = k_pc * k_у * (T_ос * ρ + 1) / 
#           [(T_му * ρ + 1) * (T_ос * ρ + 1) + k_му] * (T_k^2 * ρ^2 + T_d * ρ + 1) * (T_м * ρ + 1) * (T_оу * ρ + 1)
# W_p = h1/hзад = W_рс_p / 1 + W_рс_p

a_0 = T_му*T_ос*T_м*T_о
a_1 = T_му*T_м*T_о + T_ос*T_м*T_о + T_м*T_му*T_ос + T_о*T_му*T_ос
a_2 = T_му*T_м + T_ос*T_м + T_му*T_о + T_ос*T_о + T_м*T_о + T_му*T_ос + T_м*T_о*k_му
a_3 = T_м + T_о + T_му + T_ос + T_о*k_му + T_м*k_му
a_4 = 1 + k_му + k_рс*T_ос
a_5 = k_рс
b_4 = T_о*k_рс
b_5 = a_5